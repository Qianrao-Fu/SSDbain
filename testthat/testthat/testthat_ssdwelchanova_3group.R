...
#========================================================================================")
#To save time, omit the running SSDAOVA, but use the sample size directly.
...
eta<- cal_medbf_anova(N,m1,m2,sd,T=10000,J=1,varnames,hyp1,hyp2,ERr2,IRr2,BFthresh,eta,flag_fast=0,type,seed=10)
m<-m1
...
#test the Tables in paper
test_that("SSDANOVA", {expect_equal(eta1_test,eta1 ,tolerance = .01)})
test_that("SSDANOVA", {expect_equal(eta2_test, eta2,tolerance = .01)})
#test if min(eta1,eta2) equal to 0.8
test_that("SSDANOVA", {expect_equal(min(eta1_test,eta2_test),0.8,tolerance = .02)})

#test if equal when different set.seed is used
test_that("SSDANOVA", {expect_equal(eta_seed1,eta1 ,tolerance = .01)})
test_that("SSDANOVA", {expect_equal(eta_seed2,eta2 ,tolerance = .01)})
